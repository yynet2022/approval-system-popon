...
from .models import SimpleRequest, SimpleApprover
...
class ApprovalWorkflowTest(TestCase):
    ...
    def test_reject_action(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_PENDING, request_number="REQ-REJ"
        )
        ...
        req.refresh_from_db()
        self.assertNotEqual(req.status, SimpleRequest.STATUS_REJECTED)
    ...
    def test_withdraw_action(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_PENDING, request_number="REQ-WD"
        )
        SimpleApprover.objects.create(
            request=req, user=self.approver1, order=1
        )
        ...
        req.status = SimpleRequest.STATUS_PENDING
        req.save()
        self.client.force_login(self.approver1)
        self.client.post(withdraw_url)
        req.refresh_from_db()
    ...
    def test_proxy_remand_action(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_PENDING, request_number="REQ-PROXY"
        )
        SimpleApprover.objects.create(
            request=req, user=self.approver1, order=1
        )
        ...
        req.refresh_from_db()
        self.assertEqual(req.status, SimpleRequest.STATUS_REMANDED)
    ...
    def test_restricted_access(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            is_restricted=True, status=SimpleRequest.STATUS_PENDING,
    ...
    def test_approve_action_workflow(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_PENDING, request_number="REQ-MULTI"
        )
        SimpleApprover.objects.create(
            request=req, user=self.approver1, order=1
        )
        ...
        req.refresh_from_db()
        self.assertEqual(req.status, SimpleRequest.STATUS_APPROVED)  # 完了
    ...
    def test_remand_action(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_PENDING, request_number="REQ-REM"
        )
        SimpleApprover.objects.create(
            request=req, user=self.approver1, order=1
        )
        ...
        req.refresh_from_db()
        self.assertNotEqual(req.status, SimpleRequest.STATUS_REMANDED)
        ...
        # Approverの状態確認
        approver = SimpleApprover.objects.get(request=req, user=self.approver1)
        self.assertEqual(approver.status, SimpleApprover.STATUS_REMANDED)
    ...
    def test_resubmit_action(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_REMANDED, request_number="REQ-RESUB",
            current_step=1
        )
        app_rec = SimpleApprover.objects.create(
            request=req, user=self.approver1, order=1,
        ...
        with self.assertRaises(SimpleApprover.DoesNotExist):
            app_rec.refresh_from_db()

        # 新しいレコードが作成され、ステータスがPendingになっていることを確認
        new_approver = SimpleApprover.objects.get(
            request=req, user=self.approver1)
        self.assertEqual(new_approver.status, SimpleApprover.STATUS_PENDING)
    ...
    def test_approve_already_withdrawn(self):
        ...
        req = SimpleRequest.objects.create(
            ...
            status=SimpleRequest.STATUS_WITHDRAWN,
