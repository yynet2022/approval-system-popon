...
| **approvals** | 承認機能 | 申請モデル群, 申請CRUDビュー, 承認アクションビュー, services.py (NotificationService) |
...
* **処理**:  
  ...
  2. SimpleRequest を select\_for\_update() でロック取得。  
  3. **ルート更新 (洗い替え)**:  
     * 既存の SimpleApprover レコードを**全て削除**する。  
     * フォームで指定された承認者で SimpleApprover レコードを新規作成する。  
...
|  | **Read** | **全申請一覧 (検索)** | 全員 | / (GET) | ・is\_restrictedによるアクセス制御 ・キーワード検索、フィルタリング ・Ajaxページネーション |
|  | **Read** | **申請詳細** | 全員 | /approvals/\<uuid:pk\>/ (GET) | ・閲覧権限チェック(is\_restricted) ・関連するSimpleApprover一覧表示 ・SimpleApprovalLog（履歴）表示 |
|  | **Update** | **再申請** | 申請者 | /approvals/\<uuid:pk\>/update/ (GET/POST) | ・status=Remandedの時のみ可 ・承認ルート全洗い替え |
...
3. **【ログイン時のみ】承認依頼エリア (Pending Approvals)**:  
   * **表示条件**: ログインユーザーに紐づく SimpleApprover レコードが存在し、かつその status が Pending の案件。  
   * **場所**: お知らせエリアの下。  
...
| 7 | **申請詳細** | /approvals/\<uuid:pk\>/ | 申請内容、現在の承認状況、履歴ログ(タイムライン)の表示。 承認/差戻/取り下げ/再申請/却下ボタン等は条件に応じてここに表示。 | 全員 (制限あり) |
...
│   └── views.py                \# RequestCreateView, RequestDetailView, ActionView  
├── templates/                  \# テンプレートルート  
...
本バージョン(v3.18)の実装範囲外とし、次期以降のアップデートで対応する機能。
